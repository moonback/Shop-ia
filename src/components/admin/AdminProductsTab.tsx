...
import { generateFoodProducts, FOOD_PRODUCTS } from '../../lib/generateFoodProducts';
...
export default function AdminProductsTab({ products, categories, onRefresh }: AdminProductsTabProps) {
    ...
    const handleGenerateFoodProducts = async () => {
        ...
        if (!confirm(`Voulez-vous générer ${total} produits alimentaires ? Cette opération insère ou met à jour les produits en base (upsert sur slug).`)) return;

        setIsGeneratingCBD(true);
        setCbdProgress({ done: 0, total });

        const { success, failed, categoriesCreated } = await generateFoodProducts(
            (done, t) => setCbdProgress({ done, total: t }),
        );
        ...
        console.info(`[generateFood] ${success} produits, ${failed} échecs, ${categoriesCreated} catégories créées.`);
        if (failed > 0) alert(`Génération terminée : ${success} succès, ${failed} échec(s).`);
    };

...
export default function AdminProductsTab({ products, categories, onRefresh }: AdminProductsTabProps) {
    ...
    const openProductModal = (product?: Product) => {
        if (product) {
            ...
            setCustomSpecsText(JSON.stringify(product.attributes?.custom_specs ?? {}, null, 2));
    ...
    const handleSaveProduct = async (e: FormEvent) => {
        e.preventDefault();
        setIsSaving(true);
        let parsedCustomSpecs: Record<string, unknown> = {};
        try {
            parsedCustomSpecs = customSpecsText.trim() ? JSON.parse(customSpecsText) : {};
            if (parsedCustomSpecs === null || Array.isArray(parsedCustomSpecs) || typeof parsedCustomSpecs !== 'object') {
                alert('Les caractéristiques JSON doivent être un objet (ex: {"origin":"Sud-Ouest"}).');
                setIsSaving(false);
                return;
            }
        } catch {
            alert('JSON invalide dans Caractéristiques produit.');
            setIsSaving(false);
            return;
        }
        ...
        const payload = {
            ...
            attributes: {
                ...
                custom_specs: parsedCustomSpecs,
            },
        };
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] text-zinc-500 font-mono mt-0.5">{product.product_type || 'standard'}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-5 py-4">
                        ...
                        >
                            ...
                            <form onSubmit={handleSaveProduct} className="flex-1 overflow-y-auto p-6 space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="col-span-2">
                                        <label className={LABEL}>Caractéristiques produit (JSON)</label>
                                        <textarea
                                            rows={4}
                                            value={customSpecsText}
                                            onChange={(e) => setCustomSpecsText(e.target.value)}
                                            className={`${INPUT} font-mono text-xs`}
                                            placeholder='{"origin":"Sud-Ouest","meat":"saucisse, confit"}'
                                        />
                                        <p className="text-[10px] text-zinc-500 mt-1">
                                            Exemple : {`{"origin":"Sud-Ouest","meat":"saucisse, confit"}`}
                                        </p>
                                    </div>

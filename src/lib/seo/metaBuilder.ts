...
export function buildSEO(data: Partial<SEOData> & Pick<SEOData, 'title' | 'description'>): SEOData {
  ...
  return {
    ...
    aiEntity: data.aiEntity ?? 'Épicerie fine, Produits alimentaires, Shop-ia',
    og: {
...
export function buildProductSEO(product: Product): SEOData {
  ...
  const description = product.description ?? `Découvrez ${product.name}, un produit alimentaire de qualité disponible sur Shop-ia.`;
  ...
  return buildSEO({
    ...
    keywords: [
      'épicerie fine',
      product.name,
      product.category?.name ?? 'produit alimentaire',
      'alimentation france',
      'shop-ia',
    ],
    topic: `Produit alimentaire: ${product.name}`,
    semanticKeywords: [
      'épicerie premium',
    ...
    aiEntity: `Product:${product.name};Brand:Shop-ia;Category:${product.category?.name ?? 'Alimentaire'}`,
    og: {
      title: baseTitle,
      description,
